...
function BookEditForm({ setIsEditOpen, productToEdit }) {
  const { id, title, author, category, price, cover } = productToEdit;
  ...
  const submitHandler = async (formData) => {
    await axios.patch(`${API_URL}/books/${id}`, formData);
      ...
      <form onSubmit={handleSubmit(submitHandler)} className="ml-5" noValidate>
        ...
        <div className="flex flex-col items-center gap-1">
          <div>
            <input
              type="submit"
              value="Patvirtinti pakeitimus"
              className="btn btn-success"
            />
          </div>

          <div>
            <button className="btn btn-neutral" onClick={deleteHandler}>
              Ištrinti knygą
            </button>
          </div>
          <div>
